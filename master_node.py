...
def master_process():
    ...
    size = comm.Get_size()    #total nb of available processes
    status = MPI.Status()
    
    logging.info(f"Master node started with rank {rank} of {size}")
    
    # Initialize task queue, database connections, etc.
    # In a production system, you might use Redis, RabbitMQ, or similar technologies
    ...
    while urls_to_crawl_queue or crawler_tasks_assigned > 0:
        ...
        while urls_to_crawl_queue and crawler_tasks_assigned < crawler_nodes:  # Limit tasks to available crawler nodes for simplicity
